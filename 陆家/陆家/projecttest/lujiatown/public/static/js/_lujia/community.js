...
export function getBoundary_Community(map) {
  ...
  //map.setViewport(pointArray);    //调整视野     影响3D地图
...
export function showPop_Community(villageName, obj) {
  ...
  if (zhuangtai) {
    ...
    //map.setTilt(60);
    map.setTilt(73);
    var ply = new BMapGL.Polygon(obj.borderPoints, { strokeWeight: 4, strokeColor: "#ffffff", fillColor: "#54F8FD" });
    map.addOverlay(ply)
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      for (var i = 0; i < data.length; i++) {
        var areaInfo = data[i];
        var cameraArray = areaInfo.cameraInfoDtos;
...
export function showRealTimeCamera(areaInfo, cameraInfo) {
  ...
  if (areaInfo.areaType == 0) {
    ...
    // 处理厂显示一个title
    cameraInfo.title = cameraInfo.areaName + "(" + areaInfo.cameraInfoDtos.length + ")";
    // 处理厂批量获取监控地址
    cameraInfo2 = cameraInfo;
    var len = areaInfo.cameraInfoDtos;

    // 每个摄像机添加url、token，其他和areaInfo一致
    for (let index = 0; index < len; index++) {
      const element = res[index];
      lookInfo.cameraInfoDtos[index].url = element.url;
      lookInfo.cameraInfoDtos[index].token = element.token;
    }
