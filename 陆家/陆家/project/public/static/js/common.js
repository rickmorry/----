  ...
  if (title == "处理厂") {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        console.log(data);
        if (data.length == 0) {
          $("div.append.chulichang #rubb").text("该处理厂暂无垃圾信息")
        } else {
          console.log("有数据");
...
export function netaddSinglePoint_tou(point, labelText, xOffset, yOffset, icon, map, data) {
  ...
  var marker = new BMapGL.Marker(point, { icon: Icon });
  //marker.setAnimation(BMAP_ANIMATION_BOUNCE);//跳动效果
  var label = new BMapGL.Label(labelText + "(" + data.cameraInfos.length + ")", {
  ...
  if (cameraInfo.cameraType == 1) {
    ...
    if (newlookInfo.cameraInfos.length > 1) {
      for (let z = 0; z < newlookInfo.cameraInfos.length; z++) {
        ...
        let cc = `<div><video id='myvideo` + z + `' class='video-js' controls controlspreload='auto' width='320' height='200' data-setup='{}'><source id='video-source' src=` + newlookInfo.cameraInfos[z].viewUrl + ` type='application/x-mpegURL'></video></div>`
        $("#playWind" + z).html(cc);
        //设置资源路径
        ...
        videojs("myvideo" + z).ready(function () {
          ...
          this.on("loadstart", function () {
            console.log("开始请求数据 "+z);
          })
          this.on("progress", function () {
            console.log("正在请求数据 "+z);
          })
      ...
      //HLS摄像
      let cc = `<div><video id='my-video' class='video-js' style='margin:0 auto;'  controls controlspreload='auto' width='605' height='400' data-setup='{}' ><source id='video-source' headers='zzzzzz' src=` + cameraInfo.viewUrl + ` type='application/x-mpegURL'></video></div>`
      $("#playWind").html(cc);
      //设置资源路径
      videojs.addLanguage("zh-CN", {
        Play: "播放",
  ...
  if (cameraInfo.cameraInfos[0].cameraType == 1) {
    ...
    if (cameraInfo.cameraInfos.length > 1) {
      for (let z = 0; z < cameraInfo.cameraInfos.length; z++) {
        ...
        let cc = `<div><video id='myvideo` + z + `' class='video-js' controls controlspreload='auto' width='320' height='200' data-setup='{}'><source id='video-source' src=` + cameraInfo.cameraInfos[z].viewUrl + ` type='application/x-mpegURL'></video></div>`
        $("#playvideo" + z).html(cc);
        //设置资源路径
        videojs.addLanguage("zh-CN", {
          Play: "播放",
      ...
      if (cameraInfo.cameraInfos[0].status == 1) {
        let cc = `<div><video id='myvideo' class='video-js' style='margin:0 auto;' controls controlspreload='auto' width='605' height='400' data-setup='{}'><source id='video-source' src=` + cameraInfo.cameraInfos[0].viewUrl + ` type='application/x-mpegURL'></video></div>`
        $("#playvideo").html(cc);
        //设置资源路径
        videojs.addLanguage("zh-CN", {
          Play: "播放",
...
function cameraInit3(da) {
  ...
  //   }
  // })
  if (newlookInfo.cameraInfos.length > 0) {
    //因为token会过期,所以再次请求摄像头信息接口
    var url = '';//播放地址
    var accessToken = "";  //token
    ...
    console.log("newlookInfo");
    console.log(newlookInfo)
    if (newlookInfo.cameraInfos[0].cameraType == 1) {
      function handleError(e) {
        console.log('捕获到错误', e)
      }
      function handleSuccess() {
      ...
      //萤石云摄像
      if (newlookInfo.cameraInfos[0].areaType == 1) {
        //newlookInfo.areaType,处置点摄像头
        for (let index = 0; index < newlookInfo.cameraInfos.length; index++) {
          if (newlookInfo.cameraInfos[index].status == 1) {
            url += newlookInfo.cameraInfos[index].viewUrl;
        ...
        if (newlookInfo.cameraInfos[0].status == 1) {
          url = newlookInfo.cameraInfos[0].viewUrl;
          accessToken = newlookInfo.token
          decoder = new EZUIKit.EZUIPlayer({
            id: 'chulic',
            autoplay: true,
